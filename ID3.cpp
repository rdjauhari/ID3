...
void output_under_cutoff(double cutoff, double attribute, vector<string> current_node, string decision1, string decision2) {
...
void output_over_or_equal_cutoff(double cutoff, double attribute, vector<string> current_node, string decision1, string decision2) {
	double item_count1 = 0.0;
	double item_count2 = 0.0;
...
void split(vector<vector<double>> attributes, double num_att, vector<string> decision, double entropy_data, string decision1, string decision2) {
	...
	if (check(under, decision1, decision2) == T) {
		output_under_cutoff(split_val, split_att, under, decision1, decision2);
	}
		...
		split(under_vec_store, under_store.size(), under_store, entropy_data, decision1, decision2);
	}
	//right side
	if (check(over_equal, decision1, decision2) == T) {
		output_over_or_equal_cutoff(split_val, split_att, over_equal, decision1, decision2);
	}
		...
		split(over_equal_vec_store, over_equal_store.size(), over_equal_store, entropy_data, decision1, decision2);
	}
}
...
int main(int argc, char** argv) {
	...
	//splitting nodes
	split(attributes1, attributes.size(), decision, entropy_decision, string1, string2);
